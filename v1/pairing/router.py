...
@router.get("", dependencies=[Depends(transactional)], response_model=PairingListResponseModel)
@auth
async def get_pairings(request: Request, type: PairingSearchType):
    data = [ pairing.dto().model_dump() for pairing in Pairing.select().where(Pairing.is_deleted == False) ]
...
@router.get("/{pairing_id}", dependencies=[Depends(transactional)], response_model=PairingResponseModel)
@auth
async def get_pairing_by_id(request: Request, pairing_id: int):
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content=pairing.dto().model_dump()
        )
    except DoesNotExist:
        return JSONResponse(
