...
from torch.quantization import get_default_qconfig, prepare_qat, convert, fuse_modules


# for post training quantization
def prepare_ptq(model: Module, backend: str = "x86"):
    model.eval()
    model = model.cpu()
...
def prepare_qat(model: Module, backend: str = "x86"):
    model.train()
    model = model.cpu()
...
# after training
def converting_quantization(model: Module):
    model.eval()
    model = model.cpu()
...
def ptq_serving(
    ...
    model.load_state_dict(load(weight, map_location=device("cpu")))
    return model
...
def fuse_modules(model: Module, mode: str = "eval"):
    ...
    try:  # resnet based-model
        model = fuse_modules(model, modules)
